...
class BusinessSimulationSystem {
  constructor(gameState, saveManager, timeManager, eventBus) {
    ...
    this.metricsSystem = new OperatingMetricsSystem(); this.seatAllocator = new SeatAllocator(); this.financeSystem = new FinanceSystem(gameState, saveManager); this.unsubscribers = [];
  }
  ...
  processHour(time) {
    ...
    const activeThisHour = business.activeCohorts.reduce((sum, item) => sum + item.count, 0); const seatIncome = Math.round(business.activeCohorts.reduce((sum, item) => sum + item.count * item.hourlyRate, 0)); const productIncome = Math.round(allocation.allocations.reduce((sum, item) => { const segment = segments.find((entry) => entry.type === item.segmentType); return sum + item.count * segment.averageExtraSpend; }, 0));
