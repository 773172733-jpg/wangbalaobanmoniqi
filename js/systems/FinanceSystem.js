...
class FinanceSystem {
  ...
  settleRecurringExpenses(year, month) { const forecast = this.getCurrentMonthForecast(); const definitions = [['rent', forecast.estimatedRent, '房租'], ['electricity', forecast.estimatedElectricity, '电费'], ['network_fee', forecast.estimatedNetworkFee, '网络费']]; const results = definitions.filter((item) => item[1] > 0).map((item) => this.recordExpense({ category: item[0], amount: item[1], sourceSystem: 'finance', description: '第' + year + '年第' + month + '月' + item[2], gameDate: { year: year, month: month, day: 30 }, allowNegative: true, dedupeKey: item[0] + '_' + year + '_' + month })); return results.length ? { ok: results.every((item) => item.ok || item.duplicate), results: results, message: '固定费用结算完成。' } : { ok: true, skipped: true, message: '当前没有固定费用需要结算。' }; }
}
