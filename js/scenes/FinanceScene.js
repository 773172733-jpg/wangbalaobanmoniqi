...
class FinanceScene {
  ...
  onTouchStart(event) { if (this.tab !== 'ledger') return; const t = (event.touches || [])[0]; if (!t) return; this._gestureStartY = t.clientY; this._gestureStartLedgerScroll = this.ledgerScroll; this._isDragging = false; }
  onTouchMove(event) { if (this.tab !== 'ledger') return; const t = (event.touches || [])[0]; if (!t) return; if (this._gestureStartY === 0) return; const dy = this._gestureStartY - t.clientY; if (Math.abs(dy) > 5) this._isDragging = true; if (this._isDragging) { this.ledgerScroll = Math.max(0, this._gestureStartLedgerScroll + dy); this.requestRender(); } }
  onTouchEnd() { var w = this._isDragging; this._gestureStartY = 0; this._isDragging = false; return w; }
  ...
  drawLedger(context, box) {
    ...
    const rh = 32; const rg = 4; const rs = rh + rg; const tch = list.length * rs; const lvh = listBox.height; const ms = Math.max(0, tch - lvh + 8); if (this.ledgerScroll > ms) this.ledgerScroll = ms; if (this.ledgerScroll < 0) this.ledgerScroll = 0; context.save(); context.beginPath(); context.rect(listBox.x, listBox.y, listBox.width, listBox.height); context.clip(); list.forEach((item, index) => { const row = rect(listBox.x + 6, listBox.y + 5 + index * rs - this.ledgerScroll, listBox.width - 12, rh); if (row.y + row.height < listBox.y || row.y > listBox.y + lvh) return; context.fillStyle = index % 2 ? '#102638' : '#122c3d'; context.fillRect(row.x, row.y, row.width, row.height); context.fillStyle = item.direction === 'income' ? '#56bfa0' : '#e9855d'; context.font = 'bold 10px sans-serif'; context.textAlign = 'left'; context.fillText((item.direction === 'income' ? '+' : '-') + this.money(item.amount), row.x + 7, row.y + 20); context.fillStyle = '#dfe8e5'; context.fillText(categories.byId[item.category].name + ' · ' + item.description, row.x + 100, row.y + 20); context.fillStyle = '#718794'; context.textAlign = 'right'; context.fillText('第' + item.gameDate.year + '年' + item.gameDate.month + '月' + item.gameDate.day + '日 · ' + item.sourceSystem, row.x + row.width - 7, row.y + 20); this.inputManager.register('finance:txn:' + item.id, row, () => { this.selectedTransaction = item; this.requestRender(); }); }); context.restore(); if (tch > lvh) { const sbW = 4; const sbX = listBox.x + listBox.width - 10; const sbH = Math.max(20, lvh * lvh / tch); const sbY = listBox.y + this.ledgerScroll * lvh / tch; CanvasUtils.fillRoundedRect(context, { x: sbX, y: sbY, width: sbW, height: sbH }, 2, '#5a7d94'); }
