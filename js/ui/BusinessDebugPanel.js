...
class BusinessDebugPanel {
  constructor(system, inputManager, requestRender, saveManager, gameState) { this.system = system; this.inputManager = inputManager; this.requestRender = requestRender || function () {}; this.saveManager = saveManager || null; this.gameState = gameState || null; this.mode = 'metrics'; }
  ...
  draw(context, box) {
    ...
    const buttons = [['hour', '+1小时', () => this.system.simulateHours(1)], ['day', '+24小时', () => this.system.simulateHours(24)], ['week', '+7天', () => this.system.simulateHours(168)], ['metrics', '经营指标', () => { this.mode = 'metrics'; }], ['loss', '流失原因', () => { this.mode = 'loss'; }], ['clear', '清空今日', () => this.system.clearToday()], ['reset', '重置存档', () => this.resetSave()]];
